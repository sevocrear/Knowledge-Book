...
3. [Точность и полнота: Precision и Recall](#точность-и-полнота-precision-и-recall)
4. [Что Такое TPR и FPR](#что-такое-tpr-и-fpr)
5. [Определение ROC-кривой](#определение-roc-кривой)
6. [Определение ROC AUC и Интуиция](#определение-roc-auc-и-интуиция)
7. [Как Строить ROC и ROC AUC на Практике](#как-строить-roc-и-roc-auc-на-практике)
8. [Как Выбирать и Сравнивать Модели по ROC](#как-выбирать-и-сравнивать-модели-по-roc)
9. [Как Выбрать Порог Классификации](#как-выбрать-порог-классификации)
...
После выбора **порога** классификатор делит объекты на “предсказанные позитивы” и “предсказанные негативы”. Тогда:

### Precision (точность)

**Precision** отвечает на вопрос: *“Насколько чисты мои предсказанные позитивы?”*
...
Интуитивно:
- среди всех объектов, которые модель назвала “1”, какая доля действительно “1”.

### Recall (полнота)

**Recall** отвечает на вопрос: *“Какую долю всех настоящих позитивов я нашёл?”*
...
Интуитивно:
- среди всех объектов, которые на самом деле “1”, какая доля нашлась моделью.

Важно:
- для бинарной классификации **Recall совпадает с TPR** (True Positive Rate, “чувствительность”).
- Precision “наказывает” ложноположительные ошибки (`FP`), а Recall “наказывает” ложноотрицательные ошибки (`FN`).

Полезная мысль про порог:
- делая порог более строгим (реже говорим “1”), обычно повышаем **Precision**, но теряем **Recall**;
- делая порог менее строгим (чаще говорим “1”), обычно повышаем **Recall**, но падаем **Precision**.

Для ориентира (если тоже нужно):
- **Accuracy (достоверность)**: доля всех правильных ответов.
- **Specificity (избирательность)**: доля настоящих нулей, которые модель верно назвала “0”:
$$
...
Теперь про **точность и полноту**:
- **точность (Precision)** — “из тех, кого мы сказали яблоками, сколько на самом деле яблок”;
- **полнота (Recall)** — “сколько всех яблок мы вообще смогли найти”.
...
- **[Classification Losses: Cross-Entropy, Focal Loss](../classification-losses-cross-entropy-focal-loss/README.md)** — как функции потерь связаны с вероятностями и качеством классификаторов.
- **[Gaussian Distribution](../gaussian-distribution/README.md)** — базовая вероятность, на которой основаны многие статистические методы.
- **[Retrieval-Augmented Generation (RAG)](../retrieval-augmented-generation-rag/README.md)** — пример задач, где ранжирование и метрики качества тоже важны (хотя и другие).
...
3. **Bishop (2006)**: "Pattern Recognition and Machine Learning" — глава про оценку качества классификаторов.
4. **sklearn Documentation**: раздел `metrics`, функции `roc_curve`, `roc_auc_score`.
